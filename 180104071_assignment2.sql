...
---find faculty name who take course 'Programming' and and students name who are enrolled in this 'Programming' course also their major is 'CSE' 
--and report the ratio of students (having cse major) and 
---faculty in this specific course.

DECLARE
	countstudent int:= 0;
	countfaculty int :=0;
BEGIN
	SELECT Count(faculty.Fid) into countfaculty from faculty 
	join course on faculty.Fid = COURSE.Fid 
	where cname= 'Programming';
	
	FOR i IN (SELECT student.sname ,fName from faculty 
	join course on faculty.Fid = COURSE.Fid 
	join enrolled on course.cnum = enrolled.cnum 
	join student on enrolled.snum = student.snum 
	where cname= 'Programming' and major='CSE')
		LOOP
		...
		END LOOP;
	DBMS_OUTPUT.PUT_LINE('Total number of students');	
	DBMS_OUTPUT.PUT_LINE(countstudent);	
	DBMS_OUTPUT.PUT_LINE('Total number of Faculty taking Programming course');	
	DBMS_OUTPUT.PUT_LINE(countfaculty);	
	
	IF  (countstudent/countfaculty)<10 THEN
		DBMS_OUTPUT.PUT_LINE('Student and faculty ratio is less than 10');
	ELSIF (countstudent/countfaculty)<=15 THEN
		DBMS_OUTPUT.PUT_LINE('Student and faculty ratio is less than 15');
	ELSE 
		DBMS_OUTPUT.PUT_LINE('Student and faculty ratio is GREEATER than 15');
	END IF;
